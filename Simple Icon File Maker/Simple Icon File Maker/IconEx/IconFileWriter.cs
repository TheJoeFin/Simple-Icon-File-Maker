...
namespace IconConverter.IconEx
{
    ...
    public struct IconEntry
    {
        ...
        public override string ToString()
        {
            return string.Format(
                ...
                Planes, BitCount,
                BytesInRes, ImageOffset, Reserved);
        }
    }
    public class IconFileReader
    {
        ...
        private static IconData BuildIconData(Stream source, IconHeader header, IconEntry entry)
        {
            using (MemoryStream stream = new MemoryStream())
            {
                using (BinaryWriter writer = new BinaryWriter(stream, System.Text.Encoding.UTF8, true))
                {
                    ...
                    int offset = Marshal.SizeOf(typeof(IconHeader)) + Marshal.SizeOf(typeof(IconEntry));
                    ...
                    byte[] buffer = new byte[entry.BytesInRes];
                    ...
                    using (Bitmap image = Image.FromStream(stream) as Bitmap)
                    {
                        Icon temp = null;
                        try
                        {
        ...
        private static IEnumerable<IIconData> ReadStream(Stream stream)
        {
            ...
            {
                IconHeader header = new IconHeader(reader);

                IconEntry[] entries = new IconEntry[header.Count];
                ...
                IconData[] imageData = new IconData[header.Count];
                ...
                List<int> bitDepths = imageData.Select(b => b.BitDepth).Distinct().ToList();
                bitDepths.Sort();

                List<IIconData> result = new List<IIconData>();

                foreach (int i in bitDepths)
                {
    ...
    public class IconFileWriter
    {
        ...
        public void Save(string path)
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                using (FileStream fileStream = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None, (int)stream.Length, false))
                {
        ...
        private void SaveToStream(Stream stream)
        {
            using (BinaryWriter writer = new BinaryWriter(stream, Encoding.UTF8, true))
            {
                ...
                Dictionary<IconEntry, byte[]> imageData = new Dictionary<IconEntry, byte[]>();
                ...
                int offset = Marshal.SizeOf(typeof(IconHeader)) + images.Count * Marshal.SizeOf(typeof(IconEntry));
                ...
                // Write the Icons.
                foreach (KeyValuePair<IconEntry, byte[]> kvp in imageData)
                {
        ...
        private void ValidateImages()
        {
            ...
            List<Image> validatedImages = new List<Image>();
